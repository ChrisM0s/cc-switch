...
const AUTH_PROVIDER_CODEBUDDY_OAUTH: &str = "codebuddy_oauth";
...
fn ensure_auth_provider(auth_provider: &str) -> Result<&'static str, String> {
    match auth_provider {
        ...
        AUTH_PROVIDER_CODEBUDDY_OAUTH => Ok(AUTH_PROVIDER_CODEBUDDY_OAUTH),
...
pub async fn auth_start_login(
    ...
    codebuddy_state: State<'_, CodeBuddyOAuthState>,
...
pub async fn auth_poll_for_account(
    ...
    match auth_provider {
        ...
        AUTH_PROVIDER_CODEBUDDY_OAUTH => {
            let auth_manager = codebuddy_state.0.write().await;
            match auth_manager.poll_for_token(&device_code).await {
                Ok(account) => {
                    let default_account_id = auth_manager.get_status().await.default_account_id;
                    Ok(account.map(|account| {
