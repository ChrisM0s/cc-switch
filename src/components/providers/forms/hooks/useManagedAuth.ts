...
export function useManagedAuth(
  ...
  const startLoginMutation = useMutation({
    mutationFn: ({ targetAccountId, codebuddyOptions }: LoginRequest) =>
  ...
  // fork: CodeBuddy 登录走 beginLogin 同款 generation 流程，
  // 但携带企业版 profile 选项（站点/端点/企业 ID/UA）
  const startAuthWithCodebuddyOptions = useCallback(
    (codebuddyOptions: CodeBuddyAuthOptions) => {
      ...
      void queueBackendCancellation(previousDeviceCode).then(() => {
        if (generation !== flowGenerationRef.current) return;
        startLoginMutation.mutate({ codebuddyOptions, generation });
      });
    },
    [queueBackendCancellation, startLoginMutation, stopPolling],
  );
